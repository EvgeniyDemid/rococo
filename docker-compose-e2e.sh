...
if [ "$ARCH" = "arm64" ] || [ "$ARCH" = "aarch64" ]; then
  ...
  docker build --build-arg DOCKER=arm64v8/eclipse-temurin:21-jdk -t "${IMAGE_PREFIX}/${TEST_IMAGE_NAME}:latest" -f ./rococo-e-2-e-tests/Dockerfile .
else
  docker_arch="linux/amd64"
  docker build --build-arg DOCKER=eclipse-temurin:21-jdk -t "${IMAGE_PREFIX}/${TEST_IMAGE_NAME}:latest" -f ./rococo-e-2-e-tests/Dockerfile .
fi
